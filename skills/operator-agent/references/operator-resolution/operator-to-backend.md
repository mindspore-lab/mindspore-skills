# MindSpore Backend Lane Reference
...
Use it only after `api-to-operator.md` has already resolved the correct
`op_yaml` branch and Primitive/operator name.
...
- whether static backend support is visible for the resolved branch
- whether the ACLNN lane looks `auto_generate`, `customize`, or unsupported
- visible `pyboost` evidence
- visible `kbk` evidence
- a backend-lane conclusion that later stages can consume
...
- Do not use this reference to resolve public API identity or wrapper aliases.
...
- `./api-to-operator.md` - resolve the correct public API branch before backend
  analysis
