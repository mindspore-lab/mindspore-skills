...
Reference convention: use repository-local KBK implementations as the
...
- Do not mutate attributes in `InferShape`

### Step 3: Resize/Launch Optimization
...
### Step 4: Composite Operator Pattern (Meta DSL)
...
Code skeletons should come from current similar operators in the repository,
but **the repository's current code remains the final reference**.

### Step 5: View Host Kernel (When YAML Has `graph_view: True`)
