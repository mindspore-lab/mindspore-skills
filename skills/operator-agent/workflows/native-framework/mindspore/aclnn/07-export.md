...
Reference convention: export behavior should follow the current API-resolution
references under `references/` plus repository-local export code.
...
### Step 2.5: Interface Overload Configuration (If Multiple Same-Name Signatures Exist)
...
1. **Analyze the overload scenario**: determine which case applies (different input types / keyword-only args / old-new compatibility / symbolic alias)
