...
Use this reference when the target model resembles the provided Qwen3 patch or another Hugging Face decoder-only stack.
...
It also shows the fused RMSNorm formulation and uses `alpha = 0.01`, zero `beta`, and `max_sk_it = 20`.
...
It also validates expansion rate, Sinkhorn iteration count, and output reduction mode.
...
- the port uses `Qwen3RMSNorm -> Linear` instead of the notebook's fused RMSNorm path
...
The non-mHC path stays unchanged.
The mHC path:
...
- RoPE is computed from `inputs_embeds`, not from `hidden_states`, because `hidden_states` may become 4-D after stream expansion

### 5. Initialization

`Qwen3PreTrainedModel._init_weights` calls `reset_mhc_parameters()` for every mHC module.
...
- biases the residual matrix toward identity using diagonal `4.0` and off-diagonal `-4.0`

This is stronger than the notebook's zero-`beta` initialization and is worth preserving when porting to pretrained checkpoints.
...
- residual mappings are approximately doubly stochastic along both rows and columns
...
This matters because the config surface must be compatible with `from_pretrained(..., use_mhc=True)` rather than only manual `Config(...)` construction.

## Qwen3-specific caution

`transformers/src/transformers/models/qwen3/modeling_qwen3.py` is marked as auto-generated from `modular_qwen3.py`.
For a durable Hugging Face patch, port the same logic into `modular_qwen3.py` and regenerate the generated file according to the repository workflow.
