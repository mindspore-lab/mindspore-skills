...
Reference convention: current control-plane references live under
`references/`, and reusable templates live under `templates/`.
...
1. **PTA source review (mandatory)**: review the three key file categories in `op-plugin`
   ...
   - **ACLNN interface definition**: use repository evidence, visible headers, or user-provided ACLNN docs/snippets; do not guess from stale text alone
2. **Five-factor interface analysis (mandatory)**
...
3. **Choose the YAML strategy**
   ...
   - If an existing primitive signature must be changed, refer to similar operators in the MindSpore repository and analyze compatibility in detail
...
4. **Choose the integration path (core decision)** using `references/operator-resolution/operator-to-backend.md` plus repository evidence
...
5. **Produce a PTA difference record**: use `templates/pta-analysis-report.md` and generate a file such as `{op_name}_pta_analysis.md`
...
1. **Extract the ACLNN call chain**: extract all forward and backward `EXEC_NPU_CMD` / `aclnnXxx` calls from the PTA C++ code
2. **Inventory MindSpore coverage**: search one by one to confirm whether each sub-operator has already been integrated
3. **Produce the coverage inventory** using `templates/aclnn-callchain-analysis.md`
4. **Plan the rollout order**: leaves first, composite later; follow topological order
