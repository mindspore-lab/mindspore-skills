...
Reference convention: use repository YAML README plus the current
`operator-agent` references under `references/`.
...
**This is where the path decision from pre-checks lands in YAML**:
...
Use existing repository YAML files for similar operators as the primary
skeleton reference.
