...
Reference convention: use repository-local bprop implementations as the
authoritative source, plus current `operator-agent` references under
`references/`.
...
| Inplace backward | If input and output are the same object, **as long as one is used in backward, it must not be added to `SetUnusedInputs`**; if the backward logic needs the pre-update `self`, register **`CloneInplaceInput`** |
| KBK dynamic-shape inplace | `ib->Depend(target, inplace_call)` |
| `str` parameter gradient slot | Returning `OutZeros` for a `str` position may break KBK backward with dynamic shape; follow real framework behavior |

### Step 4: `SetUnusedInputs`

If backward does not depend on the tensor values of certain inputs, mark them as unused so memory can be released earlier.

See current repository bprop builders for the active skeleton pattern.

### Step 5: Dynamic Inputs In Graph Mode
...
1. **Directly throwing when a bprop scalar is unknown**: using `MS_EXCEPTION` when `ContainsValueAny()` is true instead of building a runtime branch.
   ...
   See `ReduceStd` bprop for the pattern. Details are in Step 5 of `./06-bprop.md`.
